...
## A brief history of the tidyverse {.vcenter}

```{r}
#| fig-align: center
#| out-height: 450px
#| out-width: 710px
## Source: https://hadley.github.io/25-tidyverse-history/
knitr::include_graphics("img/tidyverse_history.png")
```
...
## Operating on specific columns with across() and where()

Sometimes we may want to use data from only a subset of the columns.

Specific columns can be specified using `across()`:
...
iris %>% 
  summarise(
    across(c(Sepal.Length, Sepal.Width), mean)
  )
```
:::
...
For the exercise, you will want to have a look at combining `across()` and `where()`.

Hint: look at some of the examples for `where()`.
