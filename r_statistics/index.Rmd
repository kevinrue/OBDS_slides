...
- Clone <https://github.com/OBDS-Training/OBDS_environments>

- From that local clone, copy (recursively) the `r_statistics` sub-directory to your personal `obds_training` repository.

- Download (recursively) the shared folder `week5/r_statistics/data` from the CGAT cluster, into `r_statistics`.
...
- Use `renv::restore()` to restore the environment.
...
.x-large-list[
- Clone the repository <https://github.com/OBDS-Training/OBDS_environments>.

- Copy the `statistics_using_r` subdirectory to your personal repository for the course.

- Initialise the `renv` environment and use it for the exercises today.
]
...
- Visualise the evolution of weight over time for each diet.
